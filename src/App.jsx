...
// xwOBA data: try the pre-baked static file first (generated by scripts/update-xwoba.py),
// then the Vercel serverless function as a live fallback. Either gives real Savant xwoba;
// the OPS-derived fallback inside usePlayerStats only fires when both are missing the player.
let _xwobaCache=null;
async function loadXwobaJson(){
  if(_xwobaCache)return _xwobaCache;
  for(const url of ["/xwoba.json","/api/xwoba"]){
    try{
        ...
        if(missing.length>0){
          ...
          for(const chunk of chunks){
            ...
            for(const person of(pData.people||[])){
              ...
              let xwoba=null;
              // Approx wOBA from OBP/SLG: (1.7·OBP + SLG) / 3 reproduces league avg .315
              // (the prior 0.21·OBP + 0.15·SLG + 0.06 underestimated by ~40%, floored every matchup at 0.5×).
              const opsToXwoba=(obp,slg)=>(1.7*obp+slg)/3;
              if(type==="pitcher"){
                ...
                if(st){const obp=parseFloat(st.obp)||0,slg=parseFloat(st.slg)||0;if(slg>0)xwoba=opsToXwoba(obp,slg);}
              }else{
